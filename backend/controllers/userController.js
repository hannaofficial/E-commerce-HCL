...
export const registerUser = async (req, res) => {
  try {
    ...
    return res
      .status(201)
      .json({ success: true, user: { _id: user._id, email: user.email, name: user.name }, token: token }); // Changed to 201
  } catch (error) {
...
export const loginUser = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(201)
      .json({ success: true, user: { _id: user._id, email: user.email, name: user.name }, token: token });
